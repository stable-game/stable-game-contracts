...
contract BorrowerOperations is GstableBase, Ownable, CheckContract, IBorrowerOperations {
        ...
        onlyOwner
    {
        ...
        require(_borrowingFeeCollectorAddress != address(0), "Account cannot be zero address");
    ...
    function openTrove(uint _maxFeePercentage, uint _GMAmount, address _upperHint, address _lowerHint) external payable override {
        ...
        vars.ICR = GstableMath._computeCR(msg.value, vars.compositeDebt, vars.price);
        vars.NICR = GstableMath._computeNominalCR(msg.value, vars.compositeDebt);
    ...
    function withdrawGM(uint _maxFeePercentage, uint _GMAmount, address _upperHint, address _lowerHint) external override {
        _adjustTrove(msg.sender, 0, _GMAmount, true, _upperHint, _lowerHint, _maxFeePercentage);
    }
    ...
    function repayGM(uint _GMAmount, address _upperHint, address _lowerHint) external override {
        _adjustTrove(msg.sender, 0, _GMAmount, false, _upperHint, _lowerHint, 0);
    }

    function adjustTrove(uint _maxFeePercentage, uint _collWithdrawal, uint _GMChange, bool _isDebtIncrease, address _upperHint, address _lowerHint) external payable override {
        _adjustTrove(msg.sender, _collWithdrawal, _GMChange, _isDebtIncrease, _upperHint, _lowerHint, _maxFeePercentage);
    }
    ...
    */
    function _adjustTrove(address _borrower, uint _collWithdrawal, uint _GMChange, bool _isDebtIncrease, address _upperHint, address _lowerHint, uint _maxFeePercentage) internal {
        ContractsCache memory contractsCache = ContractsCache(troveManager, activePool, gmToken);
        ...
        _requireNonZeroAdjustment(_collWithdrawal, _GMChange);
        _requireTroveisActive(contractsCache.troveManager, _borrower);

        // Confirm the operation is either a borrower adjusting their own trove, or a pure ETH transfer from the Stability Pool to a trove
        assert(msg.sender == _borrower || (msg.sender == stabilityPoolAddress && msg.value > 0 && _GMChange == 0));
        ...
        vars.oldICR = GstableMath._computeCR(vars.coll, vars.debt, vars.price);
    ...
    function _triggerBorrowingFee(ITroveManager _troveManager, IGMToken _gmToken, uint _GMAmount, uint _maxFeePercentage) internal returns (uint) {
        ...
        // Send fee to borrowingFeeCollector address
    ...
    function _withdrawGM(IActivePool _activePool, IGMToken _gmToken, address _account, uint _GMAmount, uint _netDebtIncrease) internal {
    ...
    function _repayGM(IActivePool _activePool, IGMToken _gmToken, address _account, uint _GM) internal {
    ...
    function _requireNonZeroAdjustment(uint _collWithdrawal, uint _GMChange) internal view {
        require(msg.value != 0 || _collWithdrawal != 0 || _GMChange != 0, "BorrowerOps: There must be either a collateral change or a debt change");
    }
        ...
        returns (uint)
    {
        ...
        uint newICR = GstableMath._computeCR(newColl, newDebt, _price);
        return newICR;
    }
        ...
        returns (uint)
    {
        ...
        uint newTCR = GstableMath._computeCR(totalColl, totalDebt, _price);
        return newTCR;
    }
