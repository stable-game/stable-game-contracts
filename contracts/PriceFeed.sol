...
contract PriceFeed is Ownable, CheckContract, BaseMath, IPriceFeed {
    ...
    */
    ...
    /*
    * fetchPrice():
    * Returns the latest price obtained from the Oracle. Called by Gstable functions that require a current price.
    *
    * Also callable by anyone externally.
    *
    * Non-view function - it stores the last good price seen by Gstable.
    *
    * Uses a main oracle (Chainlink) and a fallback oracle (Tellor) in case Chainlink fails. If both fail, 
    * it uses the last good price seen by Gstable.
    *
    */
    ...
    */
    ...
    function _chainlinkPriceChangeAboveMax(ChainlinkResponse memory _currentResponse, ChainlinkResponse memory _prevResponse) internal pure returns (bool) {
        ...
        uint minPrice = GstableMath._min(currentScaledPrice, prevScaledPrice);
        uint maxPrice = GstableMath._max(currentScaledPrice, prevScaledPrice);
    ...
    function _bothOraclesSimilarPrice( ChainlinkResponse memory _chainlinkResponse, TellorResponse memory _tellorResponse) internal pure returns (bool) {
        ...
        uint minPrice = GstableMath._min(scaledTellorPrice, scaledChainlinkPrice);
        uint maxPrice = GstableMath._max(scaledTellorPrice, scaledChainlinkPrice);
    ...
    function _scaleChainlinkPriceByDigits(uint _price, uint _answerDigits) internal pure returns (uint) {
        /*
        * Convert the price returned by the Chainlink oracle to an 18-digit decimal for use by Gstable.
        * At date of Gstable launch, Chainlink uses an 8-digit price, but we also handle the possibility of
        * future changes.
        *
        */
        uint price;
        if (_answerDigits >= TARGET_DIGITS) {
            // Scale the returned price value down to Gstable's target precision
            price = _price.div(10 ** (_answerDigits - TARGET_DIGITS));
        }
        else if (_answerDigits < TARGET_DIGITS) {
            // Scale the returned price value up to Gstable's target precision
