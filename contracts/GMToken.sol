...
contract GMToken is CheckContract, Ownable, IGMToken {
    using SafeMath for uint256;

    uint256 private _totalSupply;
    string constant internal _NAME = "Game Money";
    string constant internal _SYMBOL = "GM";
    ...
    function addAccessControlAddresses(
        address _troveManagerAddress,
        address _stabilityPoolAddress,
        address _borrowerOperationsAddress
    ) public onlyOwner {
    ...
    // --- Functions for intra-Gstable calls ---
    ...
    (
        ...
        override
    {
        require(deadline >= now, 'GMToken: expired deadline');
        bytes32 digest = keccak256(abi.encodePacked('\x19\x01',
                         domainSeparator(), keccak256(abi.encode(
                         _PERMIT_TYPEHASH, owner, spender, amount,
                         _nonces[owner]++, deadline))));
        address recoveredAddress = ecrecover(digest, v, r, s);
        require(recoveredAddress == owner, 'GMToken: invalid signature');
    ...
    function _requireValidRecipient(address _recipient) internal view {
        require(
            ...
            "GMToken: Cannot transfer tokens directly to the GM token contract or the zero address"
        );
        require(
            !stabilityPoolAddresses[_recipient] &&
            !troveManagerAddresses[_recipient] &&
            !borrowerOperationsAddresses[_recipient],
            "GMToken: Cannot transfer tokens directly to the StabilityPool, TroveManager or BorrowerOps"
        );
    }

    function _requireCallerIsBorrowerOperations() internal view {
        require(borrowerOperationsAddresses[msg.sender], "GMToken: Caller is not one of the BorrowerOperations");
    }
    ...
    function _requireCallerIsBOorTroveMorSP() internal view {
        require(
            ...
            "GMToken: Caller is neither one of the BorrowerOperations nor one of the TroveManager nor one of the StabilityPool"
        );
    }

    function _requireCallerIsStabilityPool() internal view {
        require(stabilityPoolAddresses[msg.sender], "GMToken: Caller is not one ofthe StabilityPool");
    }

    function _requireCallerIsTroveMorSP() internal view {
        require(
            troveManagerAddresses[msg.sender] || stabilityPoolAddresses[msg.sender],
            "GMToken: Caller is neither one of the TroveManager nor one of the StabilityPool");
    }
